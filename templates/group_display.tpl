	...
	<div class="floaticon">
		{if $print_page ne 'y'}
			{if $gBitUser->hasPermission( 'p_group_edit' )}
				<a title="{tr}Edit this group{/tr}" href="{$smarty.const.GROUPS_PKG_URL}edit.php?group_id={$gContent->mInfo.group_id}">{biticon ipackage="icons" iname="accessories-text-editor" iexplain="Edit Group"}</a>
			{/if}
			{if $gBitUser->hasPermission( 'p_group_remove' )}
				<a title="{tr}Remove this group{/tr}" href="{$smarty.const.GROUPS_PKG_URL}remove_group.php?group_id={$gContent->mInfo.group_id}">{biticon ipackage="icons" iname="edit-delete" iexplain="Remove Group"}</a>
			{/if}
		{/if}<!-- end print_page -->
	</div><!-- end .floaticon -->
