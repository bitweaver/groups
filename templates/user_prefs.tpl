		...
		{else}
			{form enctype="multipart/form-data" id="joingroupform"}
				...
				<div class="form-group submit">
					{if !$gBitUser->isInGroup( $gContent->mGroupId )}
						<input type="submit" class="btn btn-default" name="join_group" value="{tr}Join{/tr}" />
					{else}
						<input type="submit" class="btn btn-default" name="save_prefs" value="{tr}Update{/tr}" />&nbsp;
						<input type="submit" class="btn btn-default" name="leave_group" value="{tr}Unsubscribe{/tr}" />
					{/if}
				</div>
			{/form}
		{/if}
